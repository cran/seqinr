...
function (codons, numcode = 1) 
{
  ...
  allaminos = s2c(c2s(SEQINR.UTIL$CODES.NCBI$CODES[numcode]))
    ...
    synonymous = lapply(codons, function(b) {
        allcodons[which(allaminos == allaminos[which(allcodons == 
            b)])]
    })
...
"synsequence" <-
function (sequence, numcode = 1, ucoweight = NULL) 
{
