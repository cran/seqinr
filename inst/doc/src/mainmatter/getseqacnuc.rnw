...
choosebank()
@
<<closesocket1,echo=F,results=hide,eval=T>>=
close(getConnection(5))
@ 
...
@
<<closesocket2,echo=F,results=hide,eval=T>>=
close(getConnection(5))
@
...
@
<<closesocket3,echo=F,results=hide,eval=T>>=
close(getConnection(5))
@
...
@
<<closesocket4,fig=F,echo=F,results=hide,eval=T>>=
showConnections()
close(getConnection(8))
close(getConnection(7))
close(getConnection(6))
close(getConnection(5))
close(getConnection(4))
showConnections()
@ 
...
It was then about \Sexpr{ifelse(exists("tqu"), round(as.numeric(tqu[3])), "???")} seconds 
...
It was then about \Sexpr{ifelse(exists("tex"), round(as.numeric(tex[3])), "???")} seconds
