...
on 30-MAR-2009 for the \texttt{getTrans()} function). This now
fixed.
...
The argument \texttt{file.out} was move to the left so that it is easier now use
it by position during function call.
